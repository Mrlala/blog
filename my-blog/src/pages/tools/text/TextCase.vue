<template>
  <div class="tool-bg">
    <div class="tool-card">
      <h2 class="tool-title">🔡 大小写转换工具</h2>
      <p class="tool-desc">支持全大写、全小写、单词首字母、句首大写等多种模式。</p>
      <div class="tool-content" style="flex-direction: column; align-items: stretch;">
        ...
        <div class="tool-btn-row">
          <button class="tool-btn" @click="toUpper">全部大写</button>
          <button class="tool-btn" @click="toLower">全部小写</button>
          <button class="tool-btn" @click="toTitle">单词首字母大写</button>
          <button class="tool-btn" @click="toSentence">句首字母大写</button>
          <button class="tool-btn" @click="toSwap">反转大小写</button>
...
function toUpper() {
  output.value = input.value.toLocaleUpperCase()
}
function toLower() {
  output.value = input.value.toLocaleLowerCase()
}
function toTitle() {
  output.value = input.value.replace(/\b\w/g, s => s.toLocaleUpperCase()).replace(/\B\w/g, s => s.toLocaleLowerCase())
}
function toSentence() {
  // 支持英文标点、中文句号、换行断句
  output.value = input.value.replace(/(^|[.!?。？！\n\r]+)\s*([a-zA-Z])/g, (m, p1, p2) => p1 + p2.toLocaleUpperCase())
}
function toSwap() {
  output.value = input.value.split('').map(c =>
    c === c.toLocaleUpperCase() ? c.toLocaleLowerCase() : c.toLocaleUpperCase()
  ).join('')
}
function clearAll() {
  input.value = ''
